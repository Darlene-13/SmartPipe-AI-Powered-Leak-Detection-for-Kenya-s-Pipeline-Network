        ...
        (set, get) => ({
            ...
            login: async (username, password) => {
                ...
                try {
                    ...
                    const msg = err.response?.data?.message ?? err.response?.data?.error ?? err.response?.data?.detail ?? "Login failed. Check your credentials.";
            ...
            register: async ({ username, password, firstName, lastName }) => {
                ...
                try {
                    ...
                    const msg = err.response?.data?.message ?? err.response?.data?.error ?? err.response?.data?.detail ?? "Registration failed. Username may already exist.";
                    set({ loading: false, error: msg });
                    throw new Error(msg);
                }
            },
